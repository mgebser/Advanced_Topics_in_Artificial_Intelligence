...
A submission should consist of modified versions of the problem encoding in [elevator.lp](./elevator.lp) and the script in [control.py](./control.py), together with a runtime comparison (documented in a separate text or PDF file) between the supplied single-shot version and your multi-shot solving solution on a suitably chosen subset of the available instances. Please submit your solution by Wednesday, December 10, 2025 via the [Project 2 Submission link](https://tc.tugraz.at/main/mod/assign/view.php?id=535595).

### Addendum

Since (re-)grounding the auxiliary program part in [next.lp](./next.lp) together with the problem encoding [elevator.lp](./elevator.lp) introduces substantial overhead in multi-shot solving, the revised [control.revised.py](./control.revised.py) script switches to a separate clingo control object to post-process schedules and produce new dynamic facts after the `solve` calls for optimization. When taking the [control.revised.py](./control.revised.py) script as basis for a multi-shot version, the grounding overhead should significantly reduce because the auxiliary `next(n)` program doesn't add rules to the (modified) [elevator.lp](./elevator.lp) encoding.
